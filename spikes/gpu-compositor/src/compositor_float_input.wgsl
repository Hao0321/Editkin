...
@group(0) @binding(0) var<storage, read> layer_pixels: array<vec4<f32>>;
...
fn blend_color(backdrop: vec3<f32>, source: vec3<f32>, mode: u32) -> vec3<f32> {
    ...
    if (mode == 10u) { return select(backdrop / max(vec3<f32>(0.000001), vec3<f32>(1.0) - source), vec3<f32>(1.0), source >= vec3<f32>(1.0)); }
    if (mode == 11u) { return select(vec3<f32>(1.0) - (vec3<f32>(1.0) - backdrop) / max(vec3<f32>(0.000001), source), vec3<f32>(0.0), source <= vec3<f32>(0.0)); }
    return source;
}
...
fn particle_random(seed: u32, index: u32, stream: u32) -> f32 { return f32(particle_hash(seed, index, stream) & 0xffffu) / 65535.0; }
...
fn sample_layer(layer_index: u32, output_x: u32, output_y: u32) -> vec4<f32> {
    ...
    if (params.enabled == 0u || params.scale <= 0.0001) { return vec4<f32>(0.0); }
    ...
    if (params.projective_enabled != 0u) {
        ...
        local = vec2<f32>(translated.x * cosine - translated.y * sine, translated.x * sine + translated.y * cosine) / params.scale + center;
    }
    if (local.x < 0.0 || local.y < 0.0 || local.x >= f32(config.width) || local.y >= f32(config.height)) { return vec4<f32>(0.0); }
    if (params.source_kind == 1u) { return particle_source(params, local); }
    let source_index = layer_index * config.pixel_count + u32(local.y) * config.width + u32(local.x);
    var color = normalize_source_alpha(layer_pixels[source_index], params.alpha_mode);
    if (params.effect_kind == 1u || params.effect_kind == 2u) {
        let rgb_code = vec3<u32>(round(clamp(color.rgb, vec3<f32>(0.0), vec3<f32>(1.0)) * 255.0));
        let luma = (rgb_code.r * 54u + rgb_code.g * 183u + rgb_code.b * 19u + 128u) >> 8u;
        ...
        color = vec4<f32>(vec3<f32>(f32(value_code) / 255.0), color.a);
    }
...
fn main(@builtin(global_invocation_id) invocation: vec3<u32>) {
    if (invocation.x >= config.width || invocation.y >= config.height) { return; }
    var color = vec4<f32>(0.0);
    for (var layer_index = 0u; layer_index < config.layer_count; layer_index += 1u) {
        let params = layer_params[layer_index];
        color = composite(color, sample_layer(layer_index, invocation.x, invocation.y), params.opacity, params.blend_mode);
    }
    output_pixels[invocation.y * config.width + invocation.x] = color;
}
