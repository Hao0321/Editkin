...
const LUT_SIZE: u32 = 65u;
const LUT_MAX: f32 = 64.0;

fn linear_rec709_to_acescct(rgb: vec3<f32>) -> vec3<f32> {
    // OCIO 2.5.2 / ACES 2 studio config: Linear Rec.709 (sRGB) -> ACEScct.
...
fn sample_tetrahedral(rgb: vec3<f32>) -> vec3<f32> {
    ...
    let low = min(vec3<u32>(63u), vec3<u32>(floor(scaled)));
...
fn pack_rgba(color: vec4<f32>) -> u32 {
    let value = vec4<u32>(floor(clamp(color, vec4<f32>(0.0), vec4<f32>(1.0)) * 255.0 + 0.5));
...
fn main(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    packed_pixels[index] = pack_rgba(vec4<f32>(sample_tetrahedral(acescct), source.a));
}
